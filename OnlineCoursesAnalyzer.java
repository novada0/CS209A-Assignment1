...
public class OnlineCoursesAnalyzer {
    ...
    public Map<String, Integer> getPtcpCountByInst() {
        return courses.stream()
            ...
            .collect(Collectors.groupingBy(o -> o.institution, LinkedHashMap::new, Collectors.summingInt(o -> o.participants)));
    }
    ...
    public List<String> searchCourses(String courseSubject, double percentAudited, double totalCourseHours) {
        return courses.stream()
            .filter(o -> o.subject.toUpperCase().contains(courseSubject.toUpperCase()) && o.percentAudited >= percentAudited && o.totalHours <= totalCourseHours)
            .map(Course::getTitle)
            .distinct()
            .sorted()
            .collect(Collectors.toList());
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
        ...
        // get average
        Map<String, Double> number_age = courses.stream().collect(
            Collectors.groupingBy(Course::getNumber,
                Collectors.averagingDouble(Course::getMedianAge)));
        Map<String, Double> number_male = courses.stream().collect(
            Collectors.groupingBy(Course::getNumber,
                Collectors.averagingDouble(Course::getPercentMale)));
        Map<String, Double> number_degree = courses.stream().collect(
            Collectors.groupingBy(Course::getNumber,
                Collectors.averagingDouble(Course::getPercentDegree)));
        // get latest(max) title of course number
        Map<String, Optional<Course>> number_latest = courses.stream()
            .collect(Collectors.groupingBy(Course::getNumber,
                Collectors.maxBy((d1, d2) -> d1.launchDate.compareTo(d2.getLaunchDate()))));

        for(String k : number_age.keySet()){
            target.add(new Similarity(number_latest.get(k).get().title, Math.pow(number_age.get(k) - age, 2) + Math.pow(number_male.get(k)-gender*100, 2)
                + Math.pow(number_degree.get(k)-isBachelorOrHigher*100, 2)));
        }
